// pptx-utils.js - generate pptx where each slide is one activity with 3x3 images
async function generateMultiSlidePPT(records, filename) {
  if (!records || !records.length) return alert('No records');
  ...
  for (const rec of records) {
    ...
    for (let r=0;r<rows;r++) {
      for (let c=0;c<cols;c++) {
        ...
        try { slide.addImage({data: photo.url || photo.data, x, y, w: imgW, h: imgH}); } catch(e){ console.error(e); }
        slide.addText(photo.desc || `Photo ${idx+1}`, {x, y: y + imgH + 0.03, w: imgW, fontSize:9, align:'center'});
      }
    }
  }
